...
public class UnosRecepataView extends JPanel {
	...
	public UnosRecepataView(UnosRecepataFrame unosRFrame) {
		...
		JScrollPane scrollTekst = new JScrollPane(tekst, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		JScrollPane scrollSastojci = new JScrollPane(panelSastojci, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		this.add(Box.createRigidArea(new Dimension(0, 20)));
		JScrollPane scrollAparati = new JScrollPane(panelAparati, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private class Controller implements ActionListener {
		...
		public void actionPerformed(ActionEvent obj) {
			if (obj.getSource() == objavi) {
				...
				UnosSastojakaFrame unosSastojaka = new UnosSastojakaFrame(unosReceptaModel);
				...
				for (JCheckBox sastojakDugme : sastojciDugmad) {
					if (sastojakDugme == obj.getSource()) {
						if (sastojakDugme.isSelected()) {
							...
							KolicinaSastojka kolS = new KolicinaSastojka(1, koliko,
									mapaDugmeSastojak.get(sastojakDugme));
